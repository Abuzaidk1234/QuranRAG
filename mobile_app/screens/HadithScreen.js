...
import { SafeAreaView,  View, Text, StyleSheet, TouchableOpacity, ScrollView, ActivityIndicator, Image, Platform, TextInput, Alert  } from 'react-native';
...
export default function HadithScreen({ navigation }) {
  ...
  useEffect(() => {
    axios.get('http://192.168.1.100:8000/hadiths/books')
      .then(response => {
  ...
  const loadChapters = (book) => {
    setLoading(true);
    axios.get(`http://192.168.1.100:8000/hadiths/${book.id}`)
      .then(response => {
  ...
  const loadHadiths = (chapter) => {
    setLoading(true);
    axios.get(`http://192.168.1.100:8000/hadiths/${selectedBook.id}/${chapter.id}`)
      .then(response => {
  ...
  const handleSearch = () => {
    ...
    setLoading(true);
    axios.get(`http://192.168.1.100:8000/hadiths/${selectedBook.id}/search/${searchQuery.trim()}`)
      .then(response => {
      ...
      .catch(error => {
        setLoading(false);
        Alert.alert("Not Found", "Could not find a hadith with that number in this book.");
      });
  };
    ...
    <View style={styles.container}>
      <View style={styles.content}>
        {loading ? (
          ...
          <ScrollView style={StyleSheet.absoluteFillObject} contentContainerStyle={{ paddingTop: 110, paddingBottom: 50, paddingHorizontal: 20 }}>
            <View style={styles.hadithContainer}>
              <View style={styles.hadithHeader}>
                <Text style={styles.hadithNumberText}>Hadith {searchedHadith.idInBook}</Text>
              </View>
              <Text style={styles.arabicText}>{searchedHadith.arabic}</Text>
              <Text style={styles.englishText}>{searchedHadith.english?.text || searchedHadith.english}</Text>
            </View>
          </ScrollView>
        ) : selectedChapter ? (
            ...
            <ScrollView style={StyleSheet.absoluteFillObject} contentContainerStyle={{ paddingTop: 110, paddingBottom: 50, paddingHorizontal: 20 }}>
              ...
              <View style={styles.searchContainer}>
                <Ionicons name="search" size={20} color="#8baeb4" style={styles.searchIcon} />
                <TextInput
              ...
              <Text style={{ color: THEME.textMuted, fontSize: 13, textAlign: 'right', marginTop: -12, marginBottom: 20, marginRight: 10, fontStyle: 'bold', color: "#8baeb4"}}>
                Valid range: {hadiths.length > 0 ? hadiths[0].idInBook : 0} to {hadiths.length > 0 ? hadiths[hadiths.length-1].idInBook : 0}
              </Text>

              {hadiths.filter(h => chapterSearchQuery === '' || String(h.idInBook) === chapterSearchQuery).map((hadith, idx) => (
              <View key={idx} style={styles.hadithContainer}>
                <View style={styles.hadithHeader}>
                  <Text style={styles.hadithNumberText}>Hadith {hadith.idInBook}</Text>
                  <TouchableOpacity onPress={() => toggleBookmark(hadith, selectedBook.name, selectedChapter.english)}>
                    <Ionicons name={bookmarks.some(b => b.id === `hadith_${selectedBook.id}_${hadith.idInBook}`) ? "bookmark" : "bookmark-outline"} size={22} color={THEME.gold} />
                  </TouchableOpacity>
                </View>
                <Text style={styles.arabicText}>{hadith.arabic}</Text>
                <Text style={styles.englishText}>{hadith.english?.text || hadith.english}</Text>
              </View>
            ))}
          </ScrollView>
        ) : selectedBook ? (
          ...
          <ScrollView style={StyleSheet.absoluteFillObject} contentContainerStyle={{ paddingTop: 110, paddingBottom: 50, paddingHorizontal: 20 }}>
            ...
            {chapters.map((chapter) => (
              <TouchableOpacity key={chapter.id} style={styles.listItemStacked} onPress={() => loadHadiths(chapter)}>
                <View style={styles.listItemHeader}>
                  ...
                  <View style={styles.listTextContainer}>
                    ...
                    {chapter.range ? <Text style={styles.listSubtitle}>{chapter.range}</Text> : null}
                  </View>
                </View>
            ...
            {books.map((book, idx) => (
              <TouchableOpacity key={book.id} style={styles.listItem} onPress={() => loadChapters(book)}>
                ...
                <View style={styles.listTextContainer}>
                  ...
                  {book.total_hadiths ? <Text style={styles.listSubtitle}>({book.total_hadiths} Hadiths)</Text> : null}
                </View>
              </TouchableOpacity>
            ))}
          </ScrollView>
      ...
      {/* Floating Header */}
      <LinearGradient
        colors={['rgba(12, 68, 82, 1)', 'rgba(12, 68, 82, 0.9)', 'rgba(12, 68, 82, 0)']}
        locations={[0, 0.6, 1]}
        ...
        {selectedBook ? (
          <TouchableOpacity onPress={goBack} style={styles.headerRow}>
            ...
            <Text style={styles.headerTitle} numberOfLines={1}>
              {searchedHadith ? `Hadith ${searchedHadith.idInBook}` : (selectedChapter ? selectedChapter.english : selectedBook.name)}
            </Text>
          </TouchableOpacity>
        ) : (
...
const styles = StyleSheet.create({
  ...
  listItemStacked: { backgroundColor: THEME.inputBg, padding: 15, borderRadius: 15, marginBottom: 10 },
  listItemHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  ...
  listArabicStacked: { color: THEME.text, fontSize: 18, textAlign: 'right', marginTop: 5 },
  ...
  searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: THEME.inputBg, borderRadius: 20, paddingHorizontal: 15, paddingVertical: 10, marginBottom: 20 },
